...
Spanish translation of `lead-generation-scraping-apis`, published as `github.com/automationbyexperts/apis-scraping-generacion-de-leads`
(local folder `all_public_actors/APIFY_GITHUB/apis-scraping-generacion-de-leads`). Same affiliate model: every Apify link
...
- A regional group sits **first** in `groups` (`espana-latam`: España y Latinoamérica), so a local tool is
